"""AdaDMD v6: Cosine LR schedule + improved training for stable longer runs.

Key improvements over v5:
1. Cosine LR schedule with warmup - prevents overshoot at 25K+
2. Lower DM loss weight (5e-5) - gentler distributional signal
3. Gradient accumulation option for effective larger batch
4. Symlink to v5c teacher pairs to avoid regeneration
"""
...
def cosine_lr(step, warmup_steps, total_steps, base_lr, min_lr_ratio=0.1):
...
def train_adadmd_v6(device="cuda:0", num_iters=35000, batch_size=8,
                    lora_rank=8, output_dir="outputs/cifar10_v6",
                    num_teacher_pairs=10000,
                    lambda_reg=1.0, lambda_dm=5e-5, lambda_dr=0.5,
                    lr_gen=2e-5, lr_lora=1e-4,
                    reg_phase_steps=5000, warmup_steps=1000):
    """Train AdaDMD v6 with cosine LR schedule."""
    ...
    # Load or generate teacher pairs (reuse from v5c if available)
    pairs_path = os.path.join(output_dir, 'teacher_pairs.pt')
    v5c_pairs = 'outputs/cifar10_v5c/teacher_pairs.pt'
    ...
    elif os.path.exists(v5c_pairs):
        print(f"Reusing teacher pairs from {v5c_pairs}...")
        shutil.copy2(v5c_pairs, pairs_path)
        pairs = torch.load(pairs_path, weights_only=True)
        noise_bank = pairs['noise']
        image_bank = pairs['images']
    else:
    ...
    print(f"Teacher pairs: {noise_bank.shape[0]}, "
          f"image range [{image_bank.min():.2f}, {image_bank.max():.2f}]")
    print(f"Teacher stats: mean={image_bank.mean():.3f}, std={image_bank.std():.3f}")

    # Create generator
    print("Setting up generator...")
    torch.save(base_unet.state_dict(), '/tmp/gen_init_v6.pt')
    unet_config = dict(base_unet.config)
    gen_unet = UNet2DModel(**unet_config)
    gen_unet.load_state_dict(torch.load('/tmp/gen_init_v6.pt', weights_only=True))
    gen_unet = gen_unet.to(device)
    gen_unet.train()
    for p in gen_unet.parameters():
        p.requires_grad_(True)
    os.unlink('/tmp/gen_init_v6.pt')
    gc.collect()
    torch.cuda.empty_cache()
    ...
    for p in lpips_fn.parameters():
        p.requires_grad_(False)
    print(f"LPIPS loaded. Memory: {torch.cuda.memory_allocated(device)/1e6:.0f} MB")
    ...
    # Optimizers (LR will be overridden by cosine schedule)
    ...
    print(f"\nStarting AdaDMD v6 distillation:")
    ...
    print(f"  Cosine LR: warmup={warmup_steps}, gen_peak={lr_gen}, lora_peak={lr_lora}")
    print(f"  lambda_dm={lambda_dm}, lambda_reg={lambda_reg} (constant)")
    sys.stdout.flush()
    ...
    for step in range(num_iters):
        # Cosine LR schedule
        current_lr_gen = cosine_lr(step, warmup_steps, num_iters, lr_gen, min_lr_ratio=0.1)
        current_lr_lora = cosine_lr(step, warmup_steps, num_iters, lr_lora, min_lr_ratio=0.1)
        for pg in opt_gen.param_groups:
            pg['lr'] = current_lr_gen
        for pg in opt_lora.param_groups:
            pg['lr'] = current_lr_lora
        ...
        # 2. Update LoRA fake score model
        ...
        if in_reg_phase:
            ...
            # Phase 2: DM + regression (constant weight)
            ...
            # Constant regression weight
            total = dm_loss_scaled + lambda_dr * dr_loss + lambda_reg * reg_loss
        ...
        log['x_std'] += x_std
        log['x_mean'] += x_mean
        log['lr_gen'] += current_lr_gen
        log['lr_lora'] += current_lr_lora
...
if __name__ == "__main__":
    ...
    parser.add_argument("--num_iters", type=int, default=35000)
    parser.add_argument("--batch_size", type=int, default=8)
    parser.add_argument("--lora_rank", type=int, default=8)
    parser.add_argument("--output_dir", type=str, default="outputs/cifar10_v6")
    parser.add_argument("--num_teacher_pairs", type=int, default=10000)
    parser.add_argument("--lambda_reg", type=float, default=1.0)
    parser.add_argument("--lambda_dm", type=float, default=5e-5)
