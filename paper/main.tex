...
We validate AdaDMD on CIFAR-10 32$\times$32 distillation using a pretrained DDPM teacher~\citep{ho2020denoising}. Training requires under 1\,GB of GPU memory on a single device. A critical discovery is that aligning the learning rate warmup duration with the regression-only training phase (both 5,000 steps) yields dramatic improvements: \textbf{28.06 FID} in one step, an 87\% improvement over MSE distillation. This phase-aligned warmup ensures the learning rate peaks exactly when distribution matching activates, then immediately decays---the DM phase never sees an increasing learning rate. The effect is non-monotonic: warmup durations that do not align with Phase 1 (200, 1,000, 3,000 steps) all perform substantially worse.
...
This phase alignment produces a $3{\times}$ FID improvement over warmup=1,000 (28.06 vs.\ 82.64). The mechanism is interpretable: during Phase 1, an increasing LR gradually strengthens LPIPS regression, building a stable generator before distribution matching begins. During Phase 2, the immediately-decaying LR prevents the noisy DM gradients (recall the density-ratio network achieves only ${\sim}50\%$ NCE accuracy) from destabilizing the generator. Misaligned warmup durations either start the DM phase with an increasing LR (warmup $>$ Phase 1), or reach peak LR too early and begin decaying before the DM loss can provide strong initial corrections.
...
\paragraph{Phase-aligned warmup scheduling.}
The most impactful finding is that warmup duration should match the regression-only training phase. With the warmup aligned to Phase 1 (both 5,000 steps), the learning rate schedule has a clean structure: monotonically increasing during regression, monotonically decreasing during distribution matching. This yields 28.06 FID---a $3{\times}$ improvement over warmup=1,000 (82.64).

The effect is \emph{non-monotonic}: warmup=3,000 (99.07) is worse than warmup=2,000 (55.45). With warmup=3,000, the LR peaks at step 3,000 (mid-Phase 1) and is already decaying when the DM loss activates at step 5,000, starting the DM phase with a sub-peak LR. Warmup=2,000 suffers similarly but to a greater degree, starting the DM phase even further from peak. Warmup=5,000 avoids this entirely: the DM phase begins at peak LR and immediately decays, providing the strongest initial distributional correction followed by smooth convergence.
...
The phase-aligned warmup is specific to our two-phase training setup and assumes the Phase 1 duration is known. Different Phase 1 lengths would require re-tuning the warmup. The non-monotonic warmup effect (3K worse than 2K) suggests the optimum may be sharp, and validation-based warmup selection would be needed for new architectures or datasets.
...
Two key next steps remain: scaling to larger models (ImageNet, Stable Diffusion) and investigating whether phase-aligned warmup transfers to non-LoRA settings. The phase alignment principle---matching warmup to the stabilization phase of training---may generalize beyond distribution matching distillation to other multi-phase training procedures where loss terms activate at different stages.
