...
\caption{FID trajectory during training. Phase 2 (DM + regression) begins at 5K steps, producing a sharp FID drop from 233 to 140 within the next 5K steps. DR-4 verification provides an additional 4.6-point improvement at 20K steps.}
...
Unpaired regression ($\text{MSE}(G(\bz), \bx_{\text{random}})$) causes severe mean collapse, with output standard deviation dropping from 0.85 to 0.07 within 5K steps. Even paired regression with L1 or smooth L1 loss collapses with paired targets. Only paired LPIPS~\citep{zhang2018unreasonable} maintains output diversity (std $> 0.4$ throughout 20K steps), confirming the mode-seeking property of perceptual losses is essential for distillation stability.
