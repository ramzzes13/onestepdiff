...
Table~\ref{tab:main_results} reports distillation results. The SmallUNet (6.6M) with 20K distillation steps achieves 222.12 FID. The HuggingFace DDPM (35.7M) with only 5K steps before mean collapse achieves 237.09. With paired LPIPS regression preventing collapse, extended training to 50K steps is feasible.
...
Model & FID & DR-4 & DR-8 \\
\midrule
SmallUNet, 20K steps & 222.12 & 219.93 & 220.10 \\
HF DDPM, 5K steps & 237.09 & 234.23 & 233.93 \\
\bottomrule
\end{tabular}
\end{table}

\paragraph{Density-ratio verification.}
Table~\ref{tab:dr_verify} and Figure~\ref{fig:dr_verify} show that density-ratio verification consistently improves FID by 2--3 points across models. The improvement saturates between $N = 4$ and $N = 8$, indicating the verifier efficiently identifies the best candidates from small pools.

\begin{figure}[t]
\centering
\includegraphics[width=\columnwidth]{figures/dr_verification.pdf}
\caption{Density-ratio verification (best-of-$N$) improves FID by 2--3 points. Improvement saturates at $N{=}4$.}
\label{fig:dr_verify}
\end{figure}
...
HF DDPM (5K) & 237.09 & 234.23 & 233.93 \\
\bottomrule
\end{tabular}
\end{table}
...
The absolute FID values (220+) are far from state-of-the-art CIFAR-10 results ($\sim$1--5 FID). Several factors contribute: (1) the SmallUNet base model (6.6M parameters, 250 epochs) is severely undertrained compared to standard EDM benchmarks~\citep{karras2022elucidating} (60M+, extensive training); (2) ablation experiments use only 5K distillation steps versus 300K+ in DMD~\citep{yin2024onestep}; (3) the HuggingFace DDPM is a basic unconditional model, not optimized for distillation. The contribution of this work is the \textit{relative} improvement from each component, which should transfer to stronger base models and larger training budgets.

\paragraph{Mean collapse and regression loss design.}
Unpaired regression ($\text{MSE}(G(\bz), \bx_{\text{random}})$) causes severe mean collapse, with output standard deviation dropping from 0.85 to 0.07 within 5K steps. Paired regression from the teacher combined with perceptual loss (LPIPS)~\citep{zhang2018unreasonable} prevents this. L1 and smooth L1 losses still collapse even with paired targets; only LPIPS maintains output diversity, confirming the mode-seeking property of perceptual losses.
...
We evaluate only on CIFAR-10 32$\times$32. Scaling to ImageNet or Stable Diffusion requires larger GPU budgets. The absolute FID values are not competitive due to base model quality and limited training, though relative improvements from each component are consistent. The density-ratio verifier adds $N{\times}$ latency at inference time.

% ============================================================
\section{Conclusion}
\label{sec:conclusion}

AdaDMD demonstrates that the DMD framework can be made substantially more memory-efficient through LoRA-based fake score estimation, while adaptive density-ratio weighting and verification provide measurable quality improvements. The LoRA adapter reduces the fake score model from 35.7M to 1.08M parameters (97\% reduction). Ablation studies confirm that adaptive weighting improves FID by 2.1 points, DR regularization matches hybrid approaches, and inference-time verification provides 2--3 additional FID points.

Validating on ImageNet and text-to-image distillation at competitive training budgets remains important future work. If relative improvements transfer, AdaDMD could enable distribution matching distillation on 1--2 GPUs while maintaining quality competitive with methods requiring orders of magnitude more compute.
...
During regression-only Phase 1, the LPIPS loss decreases from $\sim$0.20 to $\sim$0.11 over 5,000 steps while maintaining output standard deviation above 0.6, confirming paired LPIPS prevents mean collapse. The LoRA loss decreases from $\sim$0.83 to $\sim$0.11, indicating the fake score model tracks the generator distribution.

The NCE accuracy increases from 50\% to $\sim$87\% over 2,000 steps, then stabilizes. This confirms the density-ratio network reliably distinguishes real from fake noised samples, providing meaningful adaptive weights.
